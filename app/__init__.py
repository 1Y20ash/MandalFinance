...
def create_app(config_name=None):
    ...
    def handle_http_error(error):
        if error.code == 429:
            return make_response(render_template_string('<!doctype html><title>Too many requests</title><h1>Too many requests</h1><p>Please wait a moment and try again.</p>'), 429)
        message = error.description if error.code not in (401, 403) else 'You are not authorized to perform this action.'
        return make_response(render_template_string('<!doctype html><title>{{ title }}</title><h1>{{ title }}</h1><p>{{ message }}</p>', title=error.name, message=message), error.code)
    ...
    def handle_unexpected_error(error):
        ...
        return make_response(render_template_string('<!doctype html><title>Something went wrong</title><h1>Something went wrong</h1><p>Please try again later.</p>'), 500)
