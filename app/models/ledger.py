...
class Transaction(db.Model):
    ...
    __table_args__ = (
        db.CheckConstraint("transaction_type IN ('INCOME', 'EXPENSE', 'TRANSFER', 'REVERSAL')", name='ck_transactions_type'),
        db.CheckConstraint('amount > 0', name='ck_transactions_amount_positive'),
        db.CheckConstraint("payment_mode IN ('CASH', 'UPI', 'BANK_TRANSFER', 'CHEQUE', 'GATEWAY')", name='ck_transactions_payment_mode'),
        db.CheckConstraint("is_reversed = FALSE OR reversed_by_txn_id IS NOT NULL", name='ck_transactions_reversed_has_reversal'),
        db.CheckConstraint("transaction_type <> 'REVERSAL' OR external_ref LIKE 'REVERSAL-OF-%'", name='ck_transactions_reversal_reference'),
        db.Index('ix_transactions_source', 'source_module', 'source_id'),
        db.Index('ix_transactions_account_date', 'account_id', 'transaction_date'),
        Index('uq_transactions_active_source', 'source_module', 'source_id', unique=True,
              postgresql_where=text('source_id IS NOT NULL AND is_reversed = FALSE'),
              sqlite_where=text('source_id IS NOT NULL AND is_reversed = 0')),
        Index('uq_transactions_external_ref', 'external_ref', unique=True,
