...
class Expense(db.Model):
    ...
    __table_args__ = (
        ...
        db.CheckConstraint("status IN ('SUBMITTED', 'UNDER_REVIEW', 'APPROVED', 'REJECTED', 'PAID')", name='ck_expenses_status'),
        db.CheckConstraint("payment_mode IS NULL OR payment_mode IN ('CASH', 'UPI', 'BANK_TRANSFER', 'CHEQUE')", name='ck_expenses_payment_mode'),
        Index('uq_expenses_payment_ref', 'payment_ref', unique=True,
              postgresql_where=text('payment_ref IS NOT NULL'),
              sqlite_where=text('payment_ref IS NOT NULL')),
    )
