...
class DonationService:
    ...
    def record_offline_donation(event_id, donor_name, amount, payment_mode, account_id, created_by_id,
        ...
        try:
            ...
            LedgerService.record_income(
                ...
                'DONATION', donation.id, created_by_id, payment_mode=payment_mode,
                external_ref=transaction_ref, category_id=DonationService._donation_category_id(),
                event_id=event_id, commit=False,
            )
            AuditService.log_action('CREATE', 'DONATION', donation.id,
                                    f"Recorded offline donation {donation.donation_number} of ₹{decimal_amount}",
                                    commit=False)
            db.session.commit()
            return donation
        except Exception:
    ...
    def confirm_online_donation(donation_id, gateway_payment_id, gateway_signature, account_id):
        # PostgreSQL locks the donation row so concurrent browser callbacks for
        # the same payment cannot both post the financial transaction.
        ...
        try:
            ...
            LedgerService.record_income(
                ...
                description=f"Online Donation {donation.donation_number} ({donation.receipt_number})",
            ...
            AuditService.log_action('VERIFY', 'DONATION', donation.id,
