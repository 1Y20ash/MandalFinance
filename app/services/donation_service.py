...
class DonationService:
    ...
    @staticmethod
    def _generate_donation_number(): return f"DON-{datetime.utcnow().year}-{uuid4().hex[:10].upper()}"
    @staticmethod
    def _generate_receipt_number(): return f"REC-{datetime.utcnow().year}-{uuid4().hex[:10].upper()}"

    @staticmethod
    def _normalize_amount(amount):
        try:value=Decimal(str(amount)).quantize(Decimal('0.01'),rounding=ROUND_HALF_UP)
        except (InvalidOperation,ValueError,TypeError):raise ValueError('Donation amount must be a valid number.')
        if value<=Decimal('0.00'):raise ValueError('Donation amount must be greater than zero.')
        return value

    @staticmethod
    def _validate_common(event_id,donor_name,payment_mode):
        if not event_id:raise ValueError('A target event is required.')
        if not donor_name or len(donor_name.strip())<2:raise ValueError('Donor name must contain at least 2 characters.')
        if payment_mode not in DonationService.OFFLINE_MODES:raise ValueError('Invalid offline donation payment mode.')

    @staticmethod
    def _donation_category_id():
        category=TransactionCategory.query.filter_by(name='Donations',category_type='income').first()
        if not category:
            category=TransactionCategory(name='Donations',category_type='income',description='Donation income');db.session.add(category);db.session.flush()
        return category.id

    @staticmethod
    def record_offline_donation(event_id,donor_name,amount,payment_mode,account_id,created_by_id,donor_phone=None,donor_email=None,donor_address=None,pan_number=None,purpose='General Donation',transaction_ref=None,notes=None):
    ...
    def confirm_online_donation(donation_id,gateway_payment_id,gateway_signature,account_id):
        ...
        if not donation:raise ValueError('Donation not found.')
        if donation.status=='SUCCESS':return donation
        if donation.status in {'FAILED','CANCELLED'}:raise ValueError('A failed or cancelled donation cannot be confirmed.')
        if not gateway_payment_id or not gateway_signature:raise ValueError('Gateway payment ID and signature are required.')
        if Transaction.query.filter_by(external_ref=gateway_payment_id).first():raise ValueError('This gateway payment is already recorded.')
        try:
            donation.status='SUCCESS';donation.gateway_payment_id=gateway_payment_id;donation.gateway_signature=gateway_signature;donation.receipt_number=DonationService._generate_receipt_number();donation.receipt_generated_at=datetime.utcnow();db.session.flush()
