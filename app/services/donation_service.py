...
class DonationService:
    ...
    def confirm_online_donation(
        ...
        *,
        ...
        # Lock the donation row on PostgreSQL so two concurrent webhook/handler
        # requests cannot both post the same payment into the ledger.
        ...
        if not gateway_signature and not webhook_event_id:
            raise ValueError('Gateway payment signature or webhook verification is required.')
        if webhook_event_id and Donation.query.filter_by(gateway_webhook_event_id=webhook_event_id).first():
            return donation
        ...
        try:
            ...
            if webhook_event_id:
                donation.gateway_webhook_event_id = webhook_event_id
                donation.gateway_webhook_signature = webhook_signature
