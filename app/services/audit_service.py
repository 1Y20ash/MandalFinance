...
class AuditService:
    ...
    def log_action(action, entity_type, entity_id=None, description="", details=None,
        ...
        By default the entry is committed immediately for existing callers.
        Transactional services can pass commit=False so the audit record shares
        the caller's database transaction and commits atomically with it.
        """
        user_id = None
        user_email = None
