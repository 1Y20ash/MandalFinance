...
def _require_document_access(doc):
    if current_user.is_admin or doc.uploaded_by_id == current_user.id:
        return
    abort(403)
...
def download_document(doc_id):
    ...
    if not file_bytes:
        ...
        return redirect(url_for('documents.view_document', doc_id=doc.id))
    AuditService.log_action('DOWNLOAD', 'DOCUMENT', doc.id, 'Document downloaded by authorized owner.', commit=True)
    response = make_response(file_bytes)
    response.headers['Content-Type'] = doc.file_type
    response.headers['Content-Disposition'] = f'inline; filename="{StorageDriver.sanitize_filename(doc.original_filename)}"'
