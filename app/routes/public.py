...
def _verified_payment_or_raise(gateway, donation, payment_id):
    """Verify the gateway's server-side payment record before any ledger write."""
...
def confirm_online_payment():
    ...
    # Never trust the order ID returned by the browser. It must match the order
    # created and stored by our server for this donation.
    ...
    if not gateway.verify_payment_signature(donation.gateway_order_id, payment_id, signature):
        # Keep the donation PENDING. An invalid browser response must not be
        # allowed to mutate financial state or permanently deny a valid payment.
...
def payment_webhook():
    ...
    # The event ID is the durable idempotency key. The unique constraint also
    # closes the race where two identical webhook deliveries arrive together.
    ...
    try:
        ...
        db.session.rollback()
        # Another worker won the unique event-id race. The financial write is
        # also protected by the unique gateway payment/ledger references.
        return jsonify({'status': 'already processed', 'event_id': event_id}), 200
    except Exception:
        db.session.rollback()
