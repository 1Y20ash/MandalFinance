...
def approve_user(user_id):
    ...
    try:
        ...
        AuditService.log_action('APPROVE_REGISTRATION', 'USER', user.id, f"Administrator {current_user.username} approved registration for {user.username}.", commit=False)
        db.session.commit()
...
def reject_user(user_id):
    ...
    try:
        ...
        AuditService.log_action('REJECT_REGISTRATION', 'USER', user.id, f"Administrator {current_user.username} rejected registration for {user.username}.", commit=False)
        db.session.commit()
...
def assign_user_roles(user_id):
    ...
    try:
        ...
        AuditService.log_action('ROLE_CHANGE', 'USER', user.id, f"Administrator {current_user.username} changed roles for {user.username}: {old_role_names or ['<none>']} -> {new_role_names or ['<none>']}.", details={'old_roles': old_role_names, 'new_roles': new_role_names}, commit=False)
        db.session.commit()
...
def edit_role(role_id):
    ...
    AuditService.log_action('UPDATE_ROLE', 'ROLE', role.id, f"Administrator {current_user.username} updated role '{old_name}' to '{role.name}' and changed its permissions.", commit=False)
    db.session.commit()
...
def delete_role(role_id):
    ...
    AuditService.log_action('DELETE_ROLE', 'ROLE', role_id_value, f"Administrator {current_user.username} deleted role '{role_name}'.", commit=False)
...
def update_retention_policy(policy_id):
    ...
    if disposal_action not in {'REVIEW', 'ARCHIVE', 'DELETE'}:
        ...
        return redirect(url_for('admin.retention_policies'))
    if disposal_action == 'DELETE' and policy.data_category != 'USER_ACCOUNTS':
        flash('Delete disposal is restricted to the controlled user-account erasure workflow.', 'danger')
        return redirect(url_for('admin.retention_policies'))
    try:
        ...
        db.session.commit()
        flash(f'Retention policy {policy.data_category} updated: {old_days} → {retention_days} days, {old_action} → {disposal_action}.', 'success')
    except Exception:
        db.session.rollback()
...
def review_deletion_request(request_id):
    ...
    try:
        DeletionService.review(request_id, current_user, approve=action == 'APPROVE', decision_reason=decision_reason, legal_hold=action == 'HOLD', legal_hold_reason=legal_hold_reason)
