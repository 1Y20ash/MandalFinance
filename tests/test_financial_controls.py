...
def test_event_lock_blocks_new_financial_records(app):
    with app.app_context():
        admin = db.session.execute(db.select(__import__('app.models.auth', fromlist=['User']).User).where(__import__('app.models.auth', fromlist=['User']).User.username == 'admin')).scalar_one()
        event = Event.query.first()
        FinancialControlsService.lock_event(event.id, admin, 'Final accounts closed')
        category = ExpenseCategory.query.first()
        expense = Expense(expense_ref='LOCK-TEST', event_id=event.id, category_id=category.id, title='Blocked', description='Blocked after closure', amount=Decimal('10.00'), expense_date=date.today(), status='SUBMITTED', created_by_id=admin.id)
...
def test_sponsorship_receipt_posts_ledger_transaction(app):
    with app.app_context():
        admin = db.session.execute(db.select(__import__('app.models.auth', fromlist=['User']).User).where(__import__('app.models.auth', fromlist=['User']).User.username == 'admin')).scalar_one()
        event = Event.query.first(); account = Account.query.first()
        ...
        db.session.add(sponsorship); db.session.commit()
        receipt = FinancialControlsService.create_contribution_receipt('SPONSORSHIP', sponsorship.id, account.id, '250.00', 'CASH', admin)
        ...
        assert txn.source_module == 'CONTRIBUTION_RECEIPT'
        assert txn.amount == Decimal('250.00')
        assert sponsorship.received_amount == Decimal('250.00')
...
def test_correction_cannot_self_approve(app):
    with app.app_context():
        ...
        req = FinancialControlsService.request_correction('TRANSACTION', txn.id, txn.id, 'Wrong amount', admin)
        with pytest.raises(ValueError, match='Self-approval'):
            FinancialControlsService.review_correction(req.id, admin, True, 'attempt')
...
def test_reconciliation_is_decimal(app):
    with app.app_context():
        ...
        record = FinancialControlsService.reconcile_account(account.id, date.today(), '10000.00', admin)
        assert isinstance(record.difference, Decimal)
